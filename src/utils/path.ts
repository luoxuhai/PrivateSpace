...
function _format(options: PathOptions): string {
...
type Segment = undefined | string | string[] | Segment[];
...
function _join(...segments: Segment[]): string {
  const flatten = _flatten(segments).reduce(
    (previous = '', current = '') => `${previous}/${current}`,
  );
  return _normalize(flatten);
}

function _resolve(...segments: Segment[]): string {
